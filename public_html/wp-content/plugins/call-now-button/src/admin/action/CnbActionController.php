...
class CnbActionController {
	 ...
	 *
	 * @param array<string, CnbActionType> $action_types
	 * *
	 * * @return array<string, CnbActionType>
	 */
	function filter_chat_type( array $action_types ): array {
	 ...
	 * @return array<string, CnbActionType>
	 */
	function filter_booking_type( array $action_types ): array {
		/** @global Workspace|null $cnb_workspace */
		global $cnb_workspace;

		$chat_enabled = ( new CnbUtils() )->is_chat_api_enabled();

		// remove BOOKING key if chat is not enabled for this user
		if ( ! $chat_enabled ) {
			unset( $action_types['BOOKING'] );
		}
		// remove BOOKING key if there is no workspace for this user yet
		if ( $cnb_workspace === null ) {
			unset( $action_types['BOOKING'] );
		}
